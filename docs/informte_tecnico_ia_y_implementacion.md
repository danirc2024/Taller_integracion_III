...
|4.2 El fujo de tres etapas . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .|. . .<br>7|
...
|5.2 Modulos de confguracion y datos . . . . . . . . . . . . . . . . . . . . . . . . .|. . .<br>9|
|5.3 ia_defniciones.py — la funcion que evita alucinaciones desde el diseno. . . .|. . .<br>9|
|5.4 fltros.py — Etapa 2: fltrado deterministico . . . . . . . . . . . . . . . . . . .|. . .<br>9|
...
|5.9 routers/ — endpoints delgados sobre logica comun . . . . . . . . . . . . . . .|. . .<br>11|
|5.10 main.py — el punto de ensamblaje<br>. . . . . . . . . . . . . . . . . . . . . . .|. . .<br>11|
|**6.Compatibilidadentreproveedores**|**11**|
...
|6.5 Confguracion de generacion . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .|13|
...
|7.5 distancia_max: 0 no es lo mismo que “no especifcado”<br>. . . . . . . . . . . . . . .|16|
...
|8.4 Dockerfle del servicio api_gateway . . . . . . . . . . . . . . . . . . . . . . . . . . .|20|
|8.5 Variables de entorno: lo que el modulo necesita vs. lo que declara el docker-||
|compose.yml actual. . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .|21|
|8.6 Pasos de inicializacion . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .|22|
|8.7 Nota para usar esta parte con un asistente de IA . . . . . . . . . . . . . . . . . . .<br>8.8 Sintesis<br>. . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .|22<br>23|
...
|Normalizacion semantica|Unifcar nombres de productos|API Cloud (Ope-|
...
Usuario<br>Consulta en lenguaje natural<br>Extraccion de filtros<br>Gemini - function calling forzado<br>Filtrado real<br>Codigo deterministico - sin IA<br>Explicacion<br>Gemini - acotada a productos reales<br>Respuesta al usuario<br>Solo productos reales del catalogo<br>| | Con IA (Gemini) ® Sin 1A (codigo deterministico)<br><!-- End of picture text -->
...
|Etapa 1 -> Etapa 2|JSON estructurado<br>|{"palabras_clave": ["tomate", çebolla",|
||(schema fjo)|çilantro", ...]}|
...
```
|--config.py#credenciales,endpointsymodelosporproveedor
|--productos.py#accesoalcatalogo(patronrepositorio)
|--schemas.py#contratodeentrada(Pydantic)
|--ia_definiciones.py#schemadelafuncionqueelLLMdebellamar
|--filtros.py#Etapa2:filtradodeterministico+saneamiento
|--explicacion.py#Etapa3:promptcompartido+mediciondetiempo
|--utils.py#utilidadesdeencoding
|--proveedores/
||--base.py#interfazProveedorIA(elnucleodeestaparte)
||--groq_proveedor.py
||--gemini_proveedor.py
|‘--nvidia_proveedor.py
```
...
- `api_key: str | None @abstractmethod` 

- `async def extraer_filtros(self, mensaje: str) -> ResultadoExtraccion: ... @abstractmethod` 
...
|---|---|
|fltros|Dict de fltros extraidos, listo para fltrar_productos()|
...
|---|---|---|
|Gemini|toolConfg.functionCallingConfg.mode =<br>.<sup>A</sup>NY"|Forzado|
...
|---|---|---|
|Gemini|"toolConfg": {"functionCallingConfg": {"mode":<br>.<sup>A</sup>NY"}}|Si|
...
||Groq / NVIDIA|Gemini|
|---|---|---|
|Ruta hasta la tool call|data[çhoices"][0]["message"]["tool_|calls"]<br>data[çandidates"][0][çontent"]["parts"],<br>iterando por "functionCall"|
|Formato de los<br>argumentos|String JSON — requiere<br>json.loads(...)|Dict ya parseado —<br>part["functionCall"].get(.<sup>a</sup>rgs",<br>{})|
...
|Donde vive el modelo|En el body: "model":<br>self.modelo_extraccion|.../models/{GEMINI_MOD|
|Endpoint|Mismo para todos los modelos<br>del proveedor|Uno distinto por modelo (se<br>arma en confg.py)|
...
|---|---|---|
|Function calling no<br>forzado (Gemini)|toolConfg en modo AUTO deja<br>al modelo responder en texto<br>libre|toolConfg.mode:<br>.<sup>A</sup>NY"(gemini_proveedor.py)|
|Groq rechaza la tool call<br>(400)|Campo opcional con texto en<br>vez de numero; Groq valida el<br>schema en su servidor|Prevencion en el schema +<br>resca-<br>tar_fltros_de_error_groq()<br>(fltros.py)|
|Matching de categoria por<br>igualdad exacta|Producto puntual o categorias<br>combinadas en un solo string|Substring + tokenizacion,<br>_coincide_por_texto()<br>(fltros.py)|
|distancia_max: 0 fltraba|0 es un valor numerico valido|Se ignora si es <= 0|
|todo|segun el schema|(fltrar_productos())|
...
|ingredientes no<br>generalizaba a cualquier<br>pedido|Campo pensado solo para listas<br>literales de receta|Generalizado a<br>palabras_clave<br>(ia_defniciones.py)|
|Aviso de “no disponible”<br>con falsos negativos|Sin correccion ortografca,<br>“mansana” no calza con<br>“Manzana”|Implementado pero no<br>activado — deuda tecnica<br>(ver 7.10)|
...
|Aislamiento de recursos|Ninguno (corre en el equipo de desarrollo) -> sin<br>deploy.resources.limits defnido para api_gateway en el<br>docker-compose.yml actual|
...
|--docker-compose.yml
|--.env#variablesreales,noversionado
|--.env.example
|--infrastructure/
|‘--db/
|‘--init.sql#creaapi_db,scraper_db,rutas_db
|--backend/

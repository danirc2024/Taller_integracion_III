...
### B. Enfoque API Externa (OTP / Mapbox / Google Maps)
Consiste en enviar las coordenadas (Lat, Lon) a un servicio de terceros.
- **OTP (OpenTripPlanner):** Un motor escrito en C++ rapidísimo. Tiene una API pública para pruebas gratuitas (`router.project-otp.org`), o puedes montar su propia imagen de Docker si el profesor te exige que sea local.
...
Si el usuario selecciona 3 supermercados distintos y el sistema debe encontrar el "Orden óptimo de visita", entonces A* o OTP ya no bastan por sí solos.
Para esto debes:
1. Usar OTP o A* repetidas veces para crear una "Matriz de Distancias" entre todos los puntos (A-B, A-C, B-C).
...
1. Inicia completando el código de la estrategia `OTPApiStrategy` (ya creado en `app/algorithms/osmnx_strategy.py`). Consume su API pública gratuita (`requests.get`). Es la vía más rápida para probar que tu FastAPI devuelve las rutas correctamente al Frontend.
